...
export function SuccessSummarySheet({
  ...
  if (totalItems > 0) {
    if (attemptsUsed === 1) {
      headline = 'Perfect on the first try!';
          ...
          >
            <div className="mx-auto w-full max-w-[428px] rounded-t-3xl bg-white shadow-2xl border-t border-slate-200/80 px-4 pt-3 pb-5">
              ...
              <div className="bg-slate-50 rounded-2xl px-4 py-3 mb-3">
                ...
                {/* Visual result indicators - all green for success */}
                <div className="flex items-center justify-center gap-1.5 flex-wrap">
                  {Array.from({ length: totalItems }, (_, i) => (
                    <div
                      key={i}
                      className="flex-1 h-2 rounded-full transition-all duration-300 min-w-[8px] bg-[#22C55E]"
                      aria-label={`Item ${i + 1} correct`}
                    />
                  ))}
                </div>
              </div>
